...
def cascade_separate_other(job: ProcessingJob):
    """
    Cascaded Separation - Run Demucs again on the 'other' stem to extract more instruments.

    This can pull out things like:
    - Strings, brass, woodwinds that ended up in 'other'
    - Synths and pads
    - Percussion elements not captured in drums
    - Background instruments

    The second pass may find additional vocals/guitar/etc that were missed in pass 1.
    """
    other_path = job.stems.get('other')
    if not other_path or not os.path.exists(other_path):
        logger.info("No 'other' stem to cascade separate")
        return False

    # Check if 'other' has significant content
    if not check_stem_has_content(other_path):
        logger.info("'other' stem has minimal content - skipping cascade separation")
        return False

    job.stage = 'Cascade separation: analyzing "other" stem'
    job.progress = 50
    logger.info("🔄 Starting cascaded separation on 'other' stem...")

    output_path = OUTPUT_DIR / job.job_id / 'stems' / 'cascaded'
    output_path.mkdir(parents=True, exist_ok=True)

    try:
        # Run Demucs on the 'other' stem via subprocess
        # This may extract additional instruments that were mixed into 'other'
        result = subprocess.run([
            'python3', '-m', 'demucs',
            '--out', str(output_path),
            '-n', 'htdemucs_6s',
        ...
        if result.returncode != 0:
            raise Exception(f"Cascade demucs failed: {result.stderr[:500]}")
        job.progress = 52

        # Collect cascaded stem paths
        # They'll be named like: other_vocals, other_drums, etc.
        cascade_stem_dir = output_path / 'htdemucs_6s' / Path(other_path).stem

        if cascade_stem_dir.exists():
            # Convert WAV to MP3
            convert_wavs_to_mp3(cascade_stem_dir)
            job.progress = 55

            cascade_count = 0
            for stem_file in cascade_stem_dir.glob('*.mp3'):
                stem_name = stem_file.stem  # e.g., "vocals", "drums"

                # Only add if it has significant content
                if check_stem_has_content(str(stem_file)):
                    # Prefix with "other_" to indicate it came from cascaded separation
                    cascade_name = f"other_{stem_name}"
                    job.stems[cascade_name] = str(stem_file)
                    logger.info(f"  🎵 Found cascaded stem: {cascade_name}")
                    cascade_count += 1
                else:
                    logger.info(f"  ⏭️ Skipping empty cascaded stem: {stem_name}")

            if cascade_count > 0:
                logger.info(f"✅ Cascaded separation added {cascade_count} additional stems")
                return True
            else:
                logger.info("Cascaded separation didn't find significant additional content")
                return False
        else:
            logger.warning(f"Cascaded output directory not found: {cascade_stem_dir}")
            return False

    except Exception as e:
        logger.error(f"Cascaded separation failed: {e}")
        return False
...
def convert_midi_to_musicxml(job: ProcessingJob):
    ...
    try:
        from music21 import converter, instrument, metadata, tempo
    except ImportError:
        logger.warning("music21 not installed - skipping MusicXML conversion")
        return False

    job.stage = 'Converting to notation'
    job.progress = 95
    logger.info("Converting MIDI to MusicXML")
    ...
    for stem_name, midi_path in job.midi_files.items():
        try:
            ...
            # Set proper title and metadata
            score.metadata = metadata.Metadata()
            score.metadata.title = f"{song_title} - {stem_name.replace('_', ' ').title()}"
            # Get artist from job.metadata dict (not job.artist which doesn't exist)
            artist_name = job.metadata.get('artist', '') if hasattr(job, 'metadata') and job.metadata else ""
            score.metadata.composer = artist_name
            logger.info(f"  Setting metadata: title='{score.metadata.title}', artist='{artist_name}'")

            # Detect tempo from the MIDI or audio
            detected_tempo = 120  # Default
            try:
                # Try to get tempo from MIDI
                for el in score.recurse():
                    if isinstance(el, tempo.MetronomeMark):
                        detected_tempo = el.number
                        break
            except:
                pass

            # If no tempo found, try to detect from audio
            if detected_tempo == 120:
                try:
                    stem_audio_path = job.stems.get(stem_name.split('_')[0])
                    if stem_audio_path and Path(stem_audio_path).exists():
                        import librosa
                        y, sr = librosa.load(str(stem_audio_path), duration=60)
                        tempo_detected, _ = librosa.beat.beat_track(y=y, sr=sr)
                        if tempo_detected and 40 < tempo_detected < 240:
                            detected_tempo = float(tempo_detected)
                            logger.info(f"  Detected tempo: {detected_tempo:.0f} BPM")
                except Exception as e:
            ...
            stem_base = stem_name.lower().split('_')[0]
            if stem_base == 'guitar' or 'guitar' in stem_name.lower():
                for part in score.parts:
                    part.insert(0, instrument.ElectricGuitar())
            elif stem_base == 'bass' or 'bass' in stem_name.lower():
                for part in score.parts:
                    part.insert(0, instrument.ElectricBass())
            elif stem_base == 'piano' or 'piano' in stem_name.lower():
                for part in score.parts:
                    part.insert(0, instrument.Piano())
            elif stem_base == 'vocals' or 'vocal' in stem_name.lower():
                for part in score.parts:
                    part.insert(0, instrument.Vocalist())

            # Quantize for cleaner notation (snap to 16th notes)
            try:
                score.quantize(inPlace=True, quarterLengthDivisors=[4, 3])
                logger.info(f"  Quantized to 16th note grid")
            except Exception as e:
            ...
            if xml_path.exists():
                job.musicxml_files[stem_name] = str(xml_path)
                logger.info(f"✓ Created MusicXML for {stem_name}")
...
def process_audio(job: ProcessingJob, audio_path: Path, hq_vocals: bool = False, vocal_focus: bool = False, enhance_stems: bool = False, stereo_split: bool = False, gp_tabs: bool = True, chord_detection: bool = True, mdx_model: bool = False, ensemble_mode: bool = False):
    ...
    try:
        ...
        # Cascade separation: Re-process the 'other' stem to extract additional instruments
        # This is more effective than stereo splitting for finding hidden instruments
        job.stage = 'Cascade separation'
        job.progress = 48
        try:
            cascade_result = cascade_separate_other(job)
            if cascade_result:
                logger.info("✅ Cascade separation found additional instruments in 'other' stem")
            else:
                logger.info("ℹ️ No additional instruments found in 'other' stem")
        except Exception as e:
            logger.warning(f"Cascade separation failed (non-fatal): {e}")
...
def process_url_endpoint():
    ...
    if streaming_service:
        ...
        return jsonify({
            'error': 'Unsupported URL. Supported: YouTube, Spotify, Apple Music, SoundCloud, Bandcamp, Vimeo'
        }), 400
