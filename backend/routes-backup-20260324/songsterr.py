...
def _identify_chord(midi_notes):
    ...
    for root in note_names:
        ...
        # 7#9 — Hendrix chord: root + major 3rd + #9 (=b3) + minor 7th
        # Must check BEFORE major/minor since it contains both 3rd and b3rd
        if {0, 3, 4, 10} <= intervals:
            name = root + '7#9'
            priority = 4
        # Major
        elif {0, 4, 7} <= intervals:
            if 10 in intervals:
                name = root + '7'
...
def _derive_chords_from_note_data(all_track_data):
    """Derive chord progression from fret/string note data when native chord text is absent.

    Analyzes simultaneously-sounding notes beat-by-beat, collects pitch classes per measure,
    and identifies chords using music theory. Handles extended chords including 7#9.
    """
    # Use pitched (non-drum) tracks with valid standard tuning
    pitched_tracks = []
    for tdata in all_track_data:
        meta = tdata.get('_meta', {}) or {}
        instrument = meta.get('instrument', '').lower()
        tuning = meta.get('tuning') or DEFAULT_TUNING
        if not tuning or not isinstance(tuning, list):
            continue
        if 'drum' in instrument or 'percussion' in instrument:
            continue
        # Skip tracks with wildly non-standard tuning (e.g. octave effect)
        if any(t is None or t < 20 or t > 88 for t in tuning):
            continue
        pitched_tracks.append((tdata, list(tuning)))

    if not pitched_tracks:
        return []

    max_measures = max(len(t.get('measures', [])) for t, _ in pitched_tracks)
    result = []

    for mi in range(max_measures):
        sig = [4, 4]
        # beat_index -> list of (midi, note_name)
        beat_notes: dict = {}

        for tdata, tuning in pitched_tracks:
            measures = tdata.get('measures', [])
            if mi >= len(measures):
                continue
            m = measures[mi]
            ...
            for v in m.get('voices', []):
                for bi, b in enumerate(v.get('beats', [])):
                    for n in b.get('notes', []):
                        if (not n.get('rest') and
                                n.get('fret') is not None and
                                n.get('string') is not None and
                                0 <= n['string'] < len(tuning)):
                            midi = tuning[n['string']] + n['fret']
                            if 20 <= midi <= 100:
                                beat_notes.setdefault(bi, []).append(
                                    (midi, NOTE_NAMES[midi % 12])
                                )

        if not beat_notes:
            result.append({'measure': mi + 1, 'signature': sig, 'chords': []})
            continue

        # Identify chords on multi-note beats, vote by note count
        chord_votes: dict = {}
        for bi, notes in beat_notes.items():
            if len(notes) < 2:
                continue
            # Deduplicate pitch classes, keep lowest MIDI of each
            pc_map: dict = {}
            for midi, pc in notes:
                if pc not in pc_map or midi < pc_map[pc]:
                    pc_map[pc] = midi
            midi_notes = sorted((midi, pc) for pc, midi in pc_map.items())
            chord = _identify_chord(midi_notes)
            if chord:
                chord_votes[chord] = chord_votes.get(chord, 0) + len(notes)

        if chord_votes:
            chord_name = max(chord_votes, key=lambda k: chord_votes[k])
            result.append({'measure': mi + 1, 'signature': sig, 'chords': [chord_name]})
        else:
            # Fallback: accumulate all pitch classes in the measure, try whole-measure identification
            all_notes = [(midi, pc) for notes in beat_notes.values() for midi, pc in notes]
            pc_map = {}
            for midi, pc in all_notes:
                if pc not in pc_map or midi < pc_map[pc]:
                    pc_map[pc] = midi
            midi_notes = sorted((midi, pc) for pc, midi in pc_map.items())
            chord = _identify_chord(midi_notes)
            result.append({'measure': mi + 1, 'signature': sig, 'chords': [chord] if chord else []})
...
def fetch_chord_data(song_id):
    ...
    for idx in range(len(tracks_meta)):
        ...
        resp.raise_for_status()
        tdata = resp.json()
        tdata['_meta'] = tracks_meta[idx]  # attach metadata for chord derivation
        all_tracks.append(tdata)
    ...
    # Fallback: derive chords from fret/string note data when native annotations are empty
    if not any(m.get('chords') for m in chords):
        logger.info(f"No native chord annotations for song {song_id}, deriving from tab note data")
        derived = _derive_chords_from_note_data(all_tracks)
        measure_map = {m['measure']: m for m in chords}
        for dm in derived:
            if dm['chords'] and dm['measure'] in measure_map:
                measure_map[dm['measure']]['chords'] = dm['chords']
