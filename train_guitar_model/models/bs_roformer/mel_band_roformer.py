...
class MelBandRoformer(Module):
    ...
    def forward(
        ...
        # MPS doesn't support scatter_add_ for complex tensors, so handle real/imag separately
        if masks.is_complex() and masks.device.type == 'mps':
            zeros_like = torch.zeros_like(stft_repr_expanded_stems)
            masks_real = torch.zeros_like(zeros_like.real).scatter_add_(2, scatter_indices, masks.real)
            masks_imag = torch.zeros_like(zeros_like.imag).scatter_add_(2, scatter_indices, masks.imag)
        ...
        if self.zero_dc:
            # whether to dc filter - MPS doesn't support index_fill for complex
            if stft_repr.is_complex() and stft_repr.device.type == 'mps':
        ...
        # Handle stem selection for both single and multi-stem cases
        if target.ndim == 4:
            target_sel = target[:, stem_ids]
            # Squeeze stem dimension if single stem to match recon_audio shape
            if target_sel.shape[1] == 1 and recon_audio.ndim == 3:
                target_sel = rearrange(target_sel, 'b 1 s t -> b s t')
        else:
            # For single stem (3D tensor), target_sel is just target
            target_sel = target
        ...
        for window_size in self.multi_stft_resolutions_window_sizes:
            ...
            # Handle both 3D (single stem) and 4D (multi-stem) tensors
            if recon_audio.ndim == 3:
                recon_flat = rearrange(recon_audio, 'b s t -> (b s) t')
                target_flat = rearrange(target_sel, 'b s t -> (b s) t')
            else:
                recon_flat = rearrange(recon_audio, 'b n s t -> (b n s) t')
                target_flat = rearrange(target_sel, 'b n s t -> (b n s) t')
